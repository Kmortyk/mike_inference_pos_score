...
class TRTInference(object):
    ...
    def __init__(self, trt_engine_path, uff_model_path, trt_engine_datatype=trt.DataType.FLOAT, calib_dataset=None,
                 batch_size=1):
    ...
    def infer_batch(self, image_paths):
        ...
        if actual_batch_size > max_batch_size:
            raise ValueError(
                "image_paths list bigger ({}) than engine max batch size ({})".format(actual_batch_size,
                                                                                      max_batch_size))
